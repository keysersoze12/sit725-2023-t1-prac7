...
let url = 'http://localhost:3000/api/invincible';
let cat = {paht:'',title:''}
...
describe('test POST api', function(){
    it('post invincible to DB', function(done){
        request.post({url:url,form:invincible}, function(a,b,c){
            //todo asserts here
            done();
        });
    });
...
describe('test DELETE api', function(){
    it('delete a invincible', function(done){
        request.delete({url:url,form:invincible}, function(a,b,c){
            //todo asserts here
            done();
        });
    });
